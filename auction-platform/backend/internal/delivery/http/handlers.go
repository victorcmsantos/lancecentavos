...
func (h *Handler) ListAuctions(c *gin.Context) {
	...
	auctions, err := h.auctionUC.List(c.Request.Context(), limit, offset)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) PlaceBid(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, usecase.ErrAuctionNotActive), errors.Is(err, usecase.ErrBidTooLow), errors.Is(err, usecase.ErrInvalidTimestamp):
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
